...
nopCommerce 5.00 is a **modular monolith** built on ASP.NET Core 10.0 with a layered / onion-style structure. All components run in a single deployable unit (Nop.Web). Data access uses LINQ2DB; the Part 1 design committed to PostgreSQL, the reference docker-compose deploys with MSSQL (LINQ2DB supports either).
...
| Catalog/stock read | `ProductService.GetTotalStockQuantityAsync()` | Serves the web storefront - affected by stock staleness |
...
1. **Outbox mechanism**: nopCommerce writes integration events to a local table as part of the order transaction; a background task publishes them to RabbitMQ. This decouples order placement from broker availability.
2. **Stock update consumer**: A background service inside nopCommerce subscribes to `stock.updated` events from RabbitMQ and calls `ProductService.AdjustInventoryAsync()`.
...
|----------------|-------|
| `IConsumer<T>` + `IEventPublisher` | The existing in-process event system - an `IConsumer<OrderPlacedEvent>` triggers the outbox write |
| `IStartupTask` / `INopStartup` | Registers RabbitMQ background services in DI |
| FluentMigrator migrations (`src/Libraries/Nop.Data/Migrations/`) | Adds the `IntegrationEvent` outbox table |
| `IRepository<T>` | LINQ2DB-backed repository for the outbox table |
| `IProductService.AdjustInventoryAsync()` | Updates stock from WMS events |
...
| `src/Libraries/Nop.Data/Migrations/` | FluentMigrator location for the outbox table migration |
| `src/Libraries/Nop.Core/Infrastructure/INopStartup.cs` | Background service registration |
